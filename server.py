...
def api_analyze_user():
    ...
    req_bearer_token = data.get("bearer_token", "").strip()
    ...
    # Choose Tweepy client: either dynamic client from request or global client
    active_client = None
    if req_bearer_token:
        try:
            active_client = tweepy.Client(bearer_token=req_bearer_token)
        except Exception as e:
            print(f"Failed to initialize dynamic Tweepy client: {e}")
    else:
        active_client = tweepy_client  # Fallback to global client

    # Try Tweepy if not forced mock
    if not force_mock:
        if not active_client:
            return jsonify({
                "error": "Twitter Bearer Token is missing. Please set it in the API Configuration on the home page or enable Simulated Demo Mode."
            }), 400
            
        try:
            # Remove leading @ if present
            clean_username = username.lstrip('@')
            user_response = active_client.get_user(username=clean_username)
            
            if user_response.data is not None:
                user_id = user_response.data.id
                tweets_response = active_client.get_users_tweets(
                    id=user_id,
                    max_results=count,
                ...
                if tweets_response.data is not None:
                    ...
                    return jsonify({
                        "error": f"No recent tweets found for user @{clean_username}."
                    }), 404
            else:
                return jsonify({
                    "error": f"Twitter user @{clean_username} not found."
                }), 404
        except Exception as e:
            print(f"Twitter API fetch failed for {username}: {e}")
            return jsonify({
                "error": f"Twitter API error: {str(e)}. Please verify your Bearer Token or enable Simulated Demo Mode."
            }), 500
    else:
        # Fallback to mock data if mock was explicitly requested
